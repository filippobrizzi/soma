...
if( ProfileTracker x = ProfileTrackParams(26, 0)) {
    ...
    //omp_set_nested(1);
//    #pragma omp parallel
    if( ProfileTracker x = ProfileTrackParams(26, 43))
    {
//        #pragma omp sections
        if( ProfileTracker x = ProfileTrackParams(26, 45))
        {
//            #pragma omp section
...
if( ProfileTracker x = ProfileTrackParams(98, 0)) {
    //omp_set_num_threads(2);
//    #pragma omp parallel for
    if( ProfileTracker x = ProfileTrackParams(98, 101, 4 - 0))
    for(int j = 0; j < 4; j ++){
...
if( ProfileTracker x = ProfileTrackParams(112, 0)) {
    //omp_set_num_threads(2);
//    #pragma omp parallel for
    if( ProfileTracker x = ProfileTrackParams(112, 115, 4 - 0))
    for(int j = 0; j < 4; j ++){
...
{
if( ProfileTracker x = ProfileTrackParams(125, 0)) {
    int i = 0;
    double t = 0, scale=1;
...
//    #pragma omp parallel
    if( ProfileTracker x = ProfileTrackParams(125, 150))
    {
//        #pragma omp sections
        if( ProfileTracker x = ProfileTrackParams(125, 152))
        {
//            #pragma omp section
            if( ProfileTracker x = ProfileTrackParams(125, 154))
            {
...
//            #pragma omp section
            if( ProfileTracker x = ProfileTrackParams(125, 163))
            {    
                flip(gray, gray, 1);
...
{
if( ProfileTracker x = ProfileTrackParams(203, 0)) {
    int i = 0;
    double t = 0, scale=1;
    vector<Rect> faces, faces2;
    const static Scalar colors[] =
    ...
    equalizeHist( gray, gray );
//    #pragma omp parallel
    if( ProfileTracker x = ProfileTrackParams(203, 227))
    {
//        #pragma omp sections
        if( ProfileTracker x = ProfileTrackParams(203, 229))
        {
//            #pragma omp section
            if( ProfileTracker x = ProfileTrackParams(203, 231))
            {
                cascade.detectMultiScale( gray, faces,
                    1.1, 3, 0
                    //|CASCADE_FIND_BIGGEST_OBJECT
...
//            #pragma omp section
            if( ProfileTracker x = ProfileTrackParams(203, 240))
            {    
                flip(gray, gray, 1);
                cascade.detectMultiScale( gray, faces2,
                                         1.1, 2, 0
